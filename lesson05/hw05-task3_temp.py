...
Task 3:
Story
For many years, The Foundation has been encoding their messages in inconspicuous letters. Now, they are looking for
a function that will solve the decoding of the messages for them.
Rules of decoding
...
So if the first sentence has two words of 3 and 7 letters in it, we should pick the third word of the second
sentence, and the seventh of the third. We would then be done with the first sentence of the hidden message.
If there is another sentence in our letter after we are done picking the words signaled by the first sentence,
we’ll treat it as we did the first. In our example, if there was a fourth sentence, we would count the lengths of
its words as we did with the first, and pick the words in those positions in the following sentences; that would
give us the second sentence of the hidden message - and we would go on like that indefinitely.
...
Expected Output
Each sentence of the decoded hidden message must have its first word capitalised (the rest should be in lowercase)
and end with a dot ..
...
Example
'Yesterday we bumped into Laura. It had to happen, but you can\'t deny the timing couldn\'t be worse. The
"mission" to try and seduce her was a complete failure last month. By the way, she still has the ring I gave her.
